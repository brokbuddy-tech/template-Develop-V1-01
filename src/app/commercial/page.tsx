...
export default async function CommercialPage({ searchParams }: CommercialPageProps) {
  ...
  const commercialProperties = allProperties.filter(p => {
    // Base filter: Commercial types
    if (p.type !== 'Office' && p.type !== 'Retail' && p.type !== 'Industrial') return false;
    ...
    // Property types filter (if user specifically filtered by one of the commercial types)
    if (types.length > 0 && !types.includes(p.type)) return false;

    // Bedrooms filter (usually 0 for commercial, but let's keep it consistent)
    if (bedrooms) {
      ...
      <div className="w-full px-4 sm:px-6 lg:px-8 py-12">
        ...
        {commercialProperties.length === 0 && (
          <div className="text-center py-20">
            ...
            <Button variant="link" className="mt-4" onClick={() => window.location.href='/commercial'}>Clear all filters</Button>
          </div>
        )}
      </div>
    </div>
