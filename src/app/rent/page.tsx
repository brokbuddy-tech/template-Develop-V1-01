...
export default async function RentPage({ searchParams }: RentPageProps) {
  ...
  const rentProperties = allProperties.filter(p => {
    // Base filter: Rent purpose and non-commercial
    if (p.purpose !== 'Rent' || p.type === 'Office' || p.type === 'Retail' || p.type === 'Industrial') return false;
      ...
      <div className="w-full px-4 sm:px-6 lg:px-8 py-12">
        ...
        {rentProperties.length === 0 && (
          <div className="text-center py-20">
            ...
            <Button variant="link" className="mt-4" onClick={() => window.location.href='/rent'}>Clear all filters</Button>
          </div>
        )}
      </div>
    </div>
