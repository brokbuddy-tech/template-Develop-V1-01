    ...
    <SearchPageLayout initialFilters={initialFilters}>
      <div>
        ...
        <div className="w-full px-4 sm:px-6 lg:px-8">
          <ResultsHeader title={`${initialFilters.propertyType === 'COMMERCIAL' ? 'Commercial ' : 'Residential '}Properties for Rent`}>
            <h1 className="bg-muted inline-flex items-center gap-2 text-foreground font-bold text-base tracking-tight mb-4 p-3 rounded-lg">
                ...
                <span>{initialFilters.category ? initialFilters.category : (initialFilters.propertyType === 'COMMERCIAL' ? 'Commercial' : 'Properties')} for Rent</span>
            </h1>
          </ResultsHeader>
        </div>
        ...
        <div className="w-full px-4 sm:px-6 lg:px-8 py-12">
          <UniversalPropertyGrid initialPreset={{ transactionType: 'RENT' }} />
        </div>
      </div>
    </SearchPageLayout>
  );
