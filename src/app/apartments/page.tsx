...
export default async function ApartmentsPage(props: ApartmentsPageProps) {
  ...
  const { properties: apartmentProperties } = await getProperties({
    category: 'Apartment',
    q,
    minPrice,
    maxPrice,
  ...
  return (
    <div>
      ...
      <div className="w-full px-4 sm:px-6 lg:px-8">
        <ResultsHeader title="Apartments in Dubai" resultsCount={apartmentProperties.length}>
      ...
      <div className="w-full px-4 sm:px-6 lg:px-8 py-12">
        <PropertyListings properties={apartmentProperties} />
        {apartmentProperties.length > 12 && (
        ...
        {apartmentProperties.length === 0 && (
          <div className="text-center py-20">
            ...
            <Button variant="link" className="mt-4" asChild><a href='/apartments'>Clear all filters</a></Button>
          </div>
        )}
      </div>
    </div>
