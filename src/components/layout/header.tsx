...
const Logo = () => (
  <Link href="/" className="flex items-center gap-2">
    ...
    <span className="text-xl font-bold tracking-wide uppercase text-black">
      {SITE_NAME}
    </span>
  </Link>
);
      ...
      link.children ? (
        <DropdownMenu key={link.label}>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" className="flex items-center gap-1 text-gray-800 hover:text-black p-0 h-auto focus-visible:ring-0 focus-visible:ring-offset-0">
              {link.label}
        ...
        <Link
          key={link.href}
          href={link.href}
          className="text-gray-800 transition-colors hover:text-black flex items-center gap-1.5"
        >
  ...
  <Sheet>
    ...
    <SheetContent side="left" className="w-full max-w-sm flex flex-col p-0">
      ...
      <div className="mt-auto p-4 border-t space-y-4">
        <Button className="w-full bg-gradient-to-r from-[#002B5B] to-[#C5A059] text-white font-semibold hover:opacity-90 transition-opacity">
      ...
      <div className="container flex h-16 items-center justify-between">
        ...
        <div className="flex items-center gap-2 md:gap-4">
          <div className="hidden lg:flex items-center gap-2">
            <Button variant="ghost" size="icon" aria-label="Favorites">
              <Heart className="h-5 w-5 text-gray-600 hover:text-black" />
            </Button>
            <LanguageSwitcher />
          </div>
          <Button className="hidden sm:inline-flex bg-gradient-to-r from-[#002B5B] to-[#C5A059] text-white px-5 py-2 h-auto font-semibold text-sm hover:opacity-90 transition-opacity rounded-md">
            Instant Vant Valuation
          </Button>
          <MobileNav />
        </div>
